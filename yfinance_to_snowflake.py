...
def extract(symbol):
    ...
    data = download(symbol, start=date, end=end_date)
    print(data)
    # return data
...
def load(d, symbol, target_table):
    ...
    try:
        cur.execute('USE DATABASE vantage_db;')
...
with DAG(
    dag_id = 'AssignYfinanceToSnowflake',
    start_date = datetime(2024,10,2),
    catchup=False,
    tags=['ETL'],
    schedule = '30 2 * * *'
) as dag:
    target_table = "vantage_db.raw_data.stock_price"
    symbol = "TSLA"
